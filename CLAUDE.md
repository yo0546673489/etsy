...
### מערכת עיצוב (Design System) — שתי תיקיות נפרדות!

העיצוב של הפרויקט מחולק ל-2 תיקיות נפרדות. כל אחת מנהלת את העיצוב של הפלטפורמה שלה:

```
apps/web/design/          ← עיצוב האתר (Tailwind CSS + CSS variables)
apps/mobile/design/       ← עיצוב האפליקציה (StyleSheet.create + React Native)
```

**⚠️ כללים חשובים:**
- **אם אתה עובד על עיצוב האתר** — עבוד רק בתוך `apps/web/design/`. אל תיגע בדפים ישירות.
- **אם אתה עובד על פיצ'ר באתר** — ייבא קומפוננטות מ-`@/design` ואל תכתוב inline styles.
- **אם אתה עובד על עיצוב האפליקציה** — עבוד רק בתוך `apps/mobile/design/`.
- **הצבעים דומים** בשתי הפלטפורמות אבל לא זהים. כל תיקיית design היא עצמאית.
- **imports:** באתר — `import { Button, Card, tw } from '@/design'`

#### צבעי האתר (light theme):
| שם | ערך | שימוש |
|----|------|-------|
| primary | `#006d43` | כפתורים, sidebar, פעולות ראשיות |
| primary-light | `#00a86b` | hover, gradients |
| primary-dark | `#005232` | hover כהה |
| background | `#f0f2f5` | רקע כללי |
| card-bg | `#ffffff` | רקע כרטיסים |
| text-primary | `#181c22` | טקסט ראשי |
| text-muted | `#6d7a70` | טקסט משני |
| danger | `#dc2626` | שגיאות, מחיקה |
| warning | `#d97706` | אזהרות |
| info | `#1d4ed8` | מידע |

#### צבעי האפליקציה (dark theme):
| שם | ערך | שימוש |
|----|------|-------|
| primary | `#006d43` | כפתורים, פעולות ראשיות |
| background | `#0a0f0d` | רקע כללי |
| card | `#121a16` | רקע כרטיסים |
| text-primary | `#ffffff` | טקסט ראשי |
| text-secondary | `#94a3b8` | טקסט משני |
...
## ⛔ כללי עבודה למודול automation-discounts — חובה לקרוא בכל צ'אט חדש

### אסור בהחלט לגעת בשרת!
מודול זה **לא מעלה שום דבר ישירות לשרת**. לא דרך SSH, לא דרך SCP, לא דרך API, לא בשום צורה.
רק המחשב הראשי מורשה לעלות לשרת. זה כלל ברזל שאסור לשבור.

### מה עושים כשצריך לעלות משהו לשרת:

1. מסיימים את העבודה על הקוד.
2. כותבים קובץ `DEPLOY_REQUEST.md` בתוך `apps/automation-discounts/` עם:
   - מה השתנה (רשימת קבצים ותיאור)
   - מה צריך להתקין (packages חדשים)
   - מה צריך להריץ על השרת (פקודות מדויקות)
   - איך לבדוק שזה עובד
...
```
✅ יש עדכון למודול automation-discounts.
📦 השינויים עלו ל-GitHub.
📋 כל ההסבר נמצא בקובץ: apps/automation-discounts/DEPLOY_REQUEST.md
🚀 תעשה git pull ותעלה לשרת לפי ההוראות בקובץ.
```

### כללים נוספים:
- עבוד רק בתוך `apps/automation-discounts/` — אל תיגע בתיקיות אחרות.
- לפני כל עבודה: `git pull origin main`
- אחרי כל עבודה: `git add . && git commit -m "תיאור" && git push origin main`
- אל תעלה קבצי `.env` או מפתחות ל-GitHub.
- `dist/` לא ב-git — build + העתקה ידנית ל-VPS.
