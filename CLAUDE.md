...
### 3. 📱 האפליקציה — תיקיית `apps/mobile/`
אפליקציית המובייל נמצאת בתיקייה **`apps/mobile/`**.
- **טכנולוגיה:** React Native + Expo
- **עובד על:** מחשב 3
...
- **מחשב 1 (אתר):** כל הפרויקט חוץ מ-`הודעות/` ו-`apps/mobile/`
- **מחשב 2 (שרת אוטומציה VPS):** רק `הודעות/`
- **מחשב 3 (מובייל):** רק `apps/mobile/`
